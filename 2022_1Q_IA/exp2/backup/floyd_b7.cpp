...
// 定数の定義
#define GENERATION_MAX	10	// 世代交代数
#define POP_SIZE	10	// 個体集団内の個体の数
#define ELITE		1	// エリート保存戦略で残す個体の数
#define TOURNAMENT_SIZE	1	// トーナメント選択で無作為抽出する個体の数
#define MUTATE_PROB	0.01	// 突然変異確率
#define MAX_NUM	16	// フロイド問題におけるN
#define FUNCTION1	rankingSelect() //関数呼び出し
#define FUNC1EXPORT	"ランキング選択" //FUNCTION1で選択した関数の選択名をを記述
#define FUNCTION2	crossover1(&population[a],&population[b],&nextPop[i],&nextPop[i+1]) //関数呼び出し
#define FUNC2EXPORT	"1点交叉"  //FUNCTION2で選択した関数の交叉名をを記述
...
void printResult(Individual *best)
{
	...
	if(fp == 0) {
		printf("Cannot open result_list1.csv for output.\n");
		exit(1);
	}
	fclose(fp);

	fp = fopen("result_list1.csv","a");
	fprintf(fp, "%f\t%f\t%d\t%d\t%d\t%d\t%f\t%d\t%s\t%s\n", comptime,best->fitness,GENERATION_MAX,POP_SIZE,ELITE,TOURNAMENT_SIZE,MUTATE_PROB,MAX_NUM,FUNC1EXPORT,FUNC2EXPORT);
	fclose(fp);
